...
$("#frmRegistrar").on('submit',function(e)
{
   ...
   var idEstado =  $('#idestado').val();
        ...
        $.post("../ajax/categoria.php?op=guardar",
            {"id":txtId,"cat":txtCat,"idEstado":idEstado},
         function(data)
        {
...
{
	$.post("../ajax/categoria.php?op=mostrar",{id : idCategoria}, function(data, status)
	{
		data = JSON.parse(data);		
	
        
		$("#id").val(data.idCategoria);
		$("#cat").val(data.descripcionCategoria);
...
function eliminarFila(idCategoria)
{
	...
	{ // confirmamos con una pregunta si queremos eliminar
        if(result)
        {
            $.post("../ajax/categoria.php?op=eliminar", {id : idCategoria}, function(e)
            {
	            bootbox.alert(e);
	            tabla.ajax.reload();
            });
