...
if (!empty($_POST)) {
  ...
  if ($_POST['action'] == 'infoProducto') {
    ...
    $query = mysqli_query($conexion, "select idProducto 'codproducto',nombreProducto 'descripcion',precioVenta 'precio',cantidadExistencia 'existencia' from Productos where idProducto = '$producto_id';");
...
// Eliminar Producto
  if ($_POST['action'] == 'delProduct') {
    ...
    $query_delete = mysqli_query($conexion, "UPDATE Productos SET idEstado = 2 WHERE idProducto = $idproducto");
...
if ($_POST['action'] == 'addCliente') {
  ...
  $genero = $_POST['tel_cliente'];
  $fechaNacimiento = $_POST['dir_cliente'];

  $query_insert = mysqli_query($conexion, "INSERT INTO cliente(dni, nombreCliente, apellidoCliente, genero, fechaNacimiento,fechaRegistro,idEstado) VALUES ('$dni','$nombre','$apellido','$genero','$fechaNacimiento','2021-02-02 12:00:00',1)");
  if ($query_insert) {
...
if ($_POST['action'] == 'searchForDetalle') {
  ...
  if (empty($_POST['user'])){
    ...
    $query = mysqli_query($conexion, "SELECT tmp.idDetalle, tmp.token_user,
    tmp.cantidad, tmp.precio, p.idProducto, p.nombreProducto
    FROM detalleTemporal tmp INNER JOIN Productos p ON tmp.idProducto = p.idProducto
